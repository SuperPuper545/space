...
public class Bullet {
    ...
    Image img = new ImageIcon("res/bl.gif").getImage();//загрузка картинки

    public Rectangle getRect() { // получение прямоугоьников
        return new Rectangle((int) x, (int) y, 10, 22); //возвращаем конструктор с размером объекта
    }
    public Bullet(){
 ...
 // геттеры
    ...
    public void update(){
        ...
        if(Listeners.is_nip) {// если работает захват
            distX = GamePanel.aim.getX() - this.x;// разница по х
            distY = this.y - GamePanel.aim.getY();
